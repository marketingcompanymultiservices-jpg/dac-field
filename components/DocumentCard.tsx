  ...
  return (
    <article className="rounded-lg border border-dac-primary/15 bg-white p-4 shadow-sm sm:p-5">
      ...
      <dl className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <Info label="Archivo" value={document.fileName ?? document.simulatedFile ?? "Sin archivo"} />
        <Info label="Tipo" value={document.fileType ?? "No registra"} />
        <Info label="Tamano" value={document.fileSize ? formatBytes(document.fileSize) : "No registra"} />
        <Info label="Ruta Storage" value={document.storagePath ?? "Local"} />
      </dl>
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        <button
          type="button"
          onClick={onDownload}
          disabled={!canDownload || !document.storagePath}
